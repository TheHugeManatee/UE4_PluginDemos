...
public:
	...
	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Bilateral Filter"), Category = "OpenCV|ImageProcessing")
		static void Canny(const UCVUMat* src, UCVUMat* dst, int32 d, float sigmaColor, float sigmaSpace) {};

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Erode"), Category = "OpenCV|ImageProcessing")
		static void erode(const UCVUMat* src, UCVUMat* dst, int32 d, float sigmaColor, float sigmaSpace) { };

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Dilate"), Category = "OpenCV|ImageProcessing")
		static void dilate(const UCVUMat* src, UCVUMat* dst, int32 d, float sigmaColor, float sigmaSpace) {};

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Morphology Ex"), Category = "OpenCV|ImageProcessing")
		static void morphologyEx(const UCVUMat* src, UCVUMat* dst, int32 d, float sigmaColor, float sigmaSpace) {};

	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Resize"), Category = "OpenCV|ImageProcessing")
		static void resize(const UCVUMat* src, UCVUMat* dst,
			int32 dsizeX, int32 dsizeY, float fx = 0, float fy = 0,
	...
	//UFUNCTION(BlueprintCallable, meta = (DisplayName = "Warp Affine"), Category = "OpenCV|ImageProcessing")
	//	static void warpAffine(const UCVUMat* src, UCVUMat*dst, const UCVUMat* M, int dsizeX, int dsizeY,
	//		FCVInterpolationFlags flags = FCVInterpolationFlags::INTER_LINEAR,
	//		int borderMode = 0 /*cv::BORDER_CONSTANT*/,
	//		FVector4 borderValue = FVector(0.f));
	//
	//	UFUNCTION(BlueprintCallable, meta = (DisplayName = "Warp Perspective"), Category = "OpenCV|ImageProcessing")
	//	static void warpPerspective(InputArray src, OutputArray dst,
